...
public class SortGroupMemberAdapter extends BaseAdapter implements SectionIndexer {
    ...
    public View getView(final int position, View view, ViewGroup arg2) {
        ...
        if (this.list.get(position).getPic() == null) {
            if (this.list.get(position).getName().length() >= 2)
                viewHolder.pic.setImageDrawable(Utils.getCircleBg(this.list.get(position).getName().substring(this.list.get(position).getName().length() - 2), mContext));
            else
                viewHolder.pic.setImageDrawable(Utils.getCircleBg(this.list.get(position).getName(), mContext));
        } else {
            viewHolder.pic.setImageResource(R.mipmap.img_pic_test);
        }
